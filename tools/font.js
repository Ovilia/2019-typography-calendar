...
for (let line of dayInfoLines) {
    if (line && line.indexOf('#') !== 0 && line.indexOf('|') >= 0) {
        ...
        + ` --text "${story}" -c "${lightColor}" --font-size="18px" --dpr=2 --max-width=280 --line-height=0.6`);
        shell.exec(`font2img -f ./fonts/xique-juzhen.ttf -o ../src/assets/imgs/fonts/story/dark/${date}.png`
            + ` --text "${story}" -c "${darkSecondaryColor}" --font-size="18px" --dpr=2 --max-width=280 --line-height=0.6`);

        if (note) {
            shell.exec(`font2img -f ./fonts/xique-juzhen.ttf -o ../src/assets/imgs/fonts/note/dark/${date}.png`
                + ` --text "${note}" -c "${lightColor}" --font-size="14px" --dpr=2 --max-width=280 --line-height=0.6`);
            shell.exec(`font2img -f ./fonts/xique-juzhen.ttf -o ../src/assets/imgs/fonts/note/dark/${date}.png`
                + ` --text "${note}" -c "${darkNoteColor}" --font-size="14px" --dpr=2 --max-width=280 `
                + `--line-height=0.6`);
        }
